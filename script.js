...
(function(){
  ...
  monthInput.addEventListener('input',function(){
    onlyDigits(monthInput);
    coverEyes(monthInput.value.length > 0 && monthInput.type === 'password');
  });

  var monthVisible = false;
  function coverEyes(cover){
    if(paws) paws.classList.toggle('up', !!cover);
  }

  if(monthToggle){
    monthToggle.addEventListener('click',function(){
      monthVisible = !monthVisible;
      monthInput.type = monthVisible ? 'text' : 'password';
      monthToggle.setAttribute('title',monthVisible ? 'Hide month' : 'Show month');
      monthToggle.setAttribute('aria-label',monthVisible ? 'Hide month' : 'Show month');
      coverEyes(!monthVisible && monthInput.value.length > 0);
    });
  }
  ...
  function unlock(){
    ...
    if(cat) cat.classList.add('happy');
  ...
  function fail(){
    ...
    setTimeout(function(){
      if(cat) cat.classList.remove('sad');
  ...
  form.addEventListener('submit',function(e){
    ...
    var ok = dayInput.value === CODE_DAY && monthInput.value === CODE_MONTH;
    ...
    coverEyes(false);
    if(btn) btn.classList.add('dooropen');

    setTimeout(function(){
      if(btn) btn.classList.add('walking');
    },300);

    setTimeout(function(){
      if(btn) btn.classList.remove('walking','dooropen');
      unlock();
    },1100);
  });

  dayInput.addEventListener('focus',function(){ coverEyes(false); });
  monthInput.addEventListener('focus',function(){
    coverEyes(!monthVisible && monthInput.value.length > 0);
  });

  /* Idle blink + gentle gaze */
  function blink(){
    if(eyes && paws && !paws.classList.contains('up')){
      eyes.classList.add('blink');
      setTimeout(function(){ eyes.classList.remove('blink'); },130);
    }
  ...
  function updateGaze(){
    if(!dayInput) return;
    var t=Math.max(0,Math.min(1,dayInput.value.length/2));
    var dx=(t-.5)*8;
    var l=document.getElementById('pupilL'), r=document.getElementById('pupilR');
    var il=document.getElementById('irisL'), ir=document.getElementById('irisR');
    if(l){l.setAttribute('cx',78+dx);r.setAttribute('cx',122+dx);il.setAttribute('cx',78+dx);ir.setAttribute('cx',122+dx);}
  }
  dayInput.addEventListener('input',updateGaze);
  updateGaze();
